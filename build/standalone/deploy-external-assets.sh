...
log() {
  printf '[deploy-external-assets] %s\n' "$1"
}
...
usage() {
  cat <<'EOF'
Usage:
  build/standalone/deploy-external-assets.sh [options] [-- extra portainer args]
...
log "Packaging external-assets artifact..."
"$SCRIPT_DIR/package-external-assets.sh" \
  --env "$ENVIRONMENT" \
  --platform "$PLATFORM" \
  --arch "$ARCH" \
...
if [[ ! -d "$PACKAGE_DIR/public" ]]; then
  printf 'Package public directory not found: %s\n' "$PACKAGE_DIR/public" >&2
  exit 1
fi
...
rm -rf "$INSTALL_DIR/public" "$INSTALL_DIR/mustache-templates"
...
log "Starting Portainer external-assets deployment..."
