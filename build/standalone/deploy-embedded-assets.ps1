...
function Write-Info {
  ...
  Write-Host "[deploy-embedded-assets] $Message"
}
...
Write-Info "Packaging embedded-assets artifact..."
& (Join-Path $ScriptDir "package-embedded-assets.ps1") `
  -EnvName $EnvName `
  -Platform $Platform `
  -Arch $Arch `
  -OutputDir $PackageDir `
  -SkipGoGet $SkipGoGet `
  -KeepBuildWorkDir:$KeepBuildWorkDir
...
Write-Info "Starting Portainer embedded-assets deployment..."
